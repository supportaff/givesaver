        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          {error && !locked && (
            <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-center">
              {error}
              {attempts > 1 && <span className="block text-red-400 mt-0.5">{5 - attempts} attempt{5 - attempts !== 1 ? 's' : ''} remaining</span>}
            </p>
          )}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-gray-600">User ID</label>
            <input type="text" placeholder="Enter your user ID" value={username} onChange={(e) => setUsername(e.target.value)} className="input-field text-sm" autoComplete="username" autoFocus disabled={locked} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-gray-600">Password</label>
            <input type="password" placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field text-sm" autoComplete="current-password" disabled={locked} />
          </div>
          <button type="submit" disabled={loading || !username || !password || locked} className="btn-primary py-3 mt-1 disabled:opacity-40 font-semibold">
