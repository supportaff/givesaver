          ...
          {loading ? (
                ...
                <tbody className="divide-y divide-gray-50">
                  ...
                  {donations.map((d) => (
                    <tr key={d.id} className={`hover:bg-gray-50 transition-colors ${ d.flagged ? 'bg-red-50' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`badge text-xs ${STATUS_COLORS[d.status] ?? 'bg-gray-100 text-gray-500'}`}>{d.status}</span>
                      </td>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {claims.length === 0 && (
                    <tr><td colSpan={6} className="text-center py-10 text-gray-400">No claims yet</td></tr>
                  )}
                  {claims.map((c) => (
