...
export default function AdminLoginPage() {
  ...
  const next         = searchParams.get('next') ?? '/__admin';
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" placeholder="Enter your user ID" value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="input-field text-sm" autoComplete="username" autoFocus disabled={locked} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-gray-600">Password</label>
            <input type="password" placeholder="Enter your password" value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="input-field text-sm" autoComplete="current-password" disabled={locked} />
          </div>
