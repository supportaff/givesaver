...
export default function MyDonationsPage() {
  ...
  async function handleLookup(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || 'error' in data) { setError('Failed to fetch donations'); return; }
      setResults(data);
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-white border-b">
        <div className="section-wrapper py-10">
          ...
          <p className="text-gray-500 mt-2">Enter your phone number to see all your listings</p>
        </div>
      </div>
                    ...
                    {results.map((d) => (
                      <div key={d.id} className="card hover:shadow-md transition-all">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {/* Copy manage link */}
                          <button
                            onClick={() => navigator.clipboard.writeText(`https://givesaver.in/manage/${d.id}`)}
                            className="btn-secondary text-xs px-4 py-2">
                            🔗 Copy Link
                          </button>
                        </div>
