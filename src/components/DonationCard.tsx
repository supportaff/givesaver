...
type ClaimForm = { name: string; phone: string; telegram: string };
...
export default function DonationCard({ donation: d }: { donation: DonationRow }) {
  ...
  const [form, setForm]           = useState<ClaimForm>({ name: '', phone: '', telegram: '' });
  const [claiming,  setClaiming]  = useState(false);
  const [claimed,   setClaimed]   = useState<{ claimId: string } | null>(null);
  const [claimErr,  setClaimErr]  = useState('');

  async function submitClaim() {
    if (!form.name || !form.phone || !form.telegram) { setClaimErr('All fields required'); return; }
    ...
    try {
      const res  = await fetch('/api/claims/init', {
        ...
        body: JSON.stringify({
          ...
          receiverTelegram: form.telegram,
        }),
      });
      const data = await res.json() as { claimId?: string; error?: string };
      if (!res.ok) { setClaimErr(data.error ?? 'Failed'); return; }
      setClaimed({ claimId: data.claimId! });
    } catch { setClaimErr('Something went wrong'); }
  ...
  return (
    <div className="card hover:shadow-md transition-all duration-200 flex flex-col gap-3 group">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
        <div className="flex items-center gap-1.5"><span className="text-gray-300">📦</span><span className="text-gray-500">Qty:</span><span className="font-medium text-gray-700">{d.quantity}</span></div>
        <div className="flex items-center gap-1.5"><span className="text-gray-300">📍</span><span className="text-gray-500">City:</span><span className="font-medium text-gray-700">{d.city}</span></div>
      </div>
      ...
      {d.status === 'AVAILABLE' && (
        <div className="flex flex-col gap-2 mt-1">
          ...
          <a href={waURL} target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl font-semibold text-sm bg-[#25D366] hover:bg-[#20b958] text-white transition-all shadow-sm">
          ...
          {!showClaim && !claimed && (
            <button onClick={() => setShowClaim(true)}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl font-semibold text-sm bg-[#229ED9] hover:bg-[#1a8fc7] text-white transition-all shadow-sm">
              🔑 Claim via Telegram OTP
            </button>
          )}
          ...
          {showClaim && !claimed && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex flex-col gap-3">
              <p className="text-sm font-semibold text-blue-800">🔑 Claim This Donation</p>
              <p className="text-xs text-blue-600">You and the donor will each receive a unique OTP on Telegram. Both must verify at pickup.</p>
              {claimErr && <p className="text-xs text-red-600">{claimErr}</p>}
              <input placeholder="Your Full Name *" value={form.name}
                onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
                className="input-field text-sm" />
              <input placeholder="Your Phone Number *" value={form.phone}
                onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))}
                className="input-field text-sm" />
              <input placeholder="Your Telegram Username (e.g. @yourname) *" value={form.telegram}
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowClaim(false)} className="btn-secondary text-sm flex-1">Cancel</button>
                <button onClick={submitClaim} disabled={claiming} className="btn-primary text-sm flex-1 disabled:opacity-40">
          ...
          {claimed && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
              <p className="text-green-700 font-semibold text-sm">✅ OTPs sent to both Telegram accounts!</p>
              <p className="text-xs text-gray-500 mt-1">Show your OTP to the donor at pickup. The donor will enter your OTP to confirm.</p>
              <a href={`/claim/${claimed.claimId}/verify`}
                className="mt-3 inline-block text-xs text-blue-600 underline">
                Enter donor’s OTP here →
              </a>
            </div>
          )}
        </div>
      ...
      {d.status !== 'AVAILABLE' && (
        <div className="text-center py-2">
          <span className={`badge text-sm ${statusMeta?.color}`}>{d.status === 'CLAIMED' ? '🤝 Claimed' : '📦 Collected'}</span>
        </div>
      )}
    </div>
  );
